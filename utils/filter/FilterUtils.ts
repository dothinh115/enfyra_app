...
export function getTargetTableNameForGroup(
  ...
  for (const relationName of relationPath) {
    ...
    if (!targetTableName && relation.targetTable?.id) {
      const targetTable = Object.values(schemas).find((schema: any) => schema.id === relation.targetTable.id);
...
export function getCombinedOptionsForContext(
  ...
  // Add columns from the context table
  schema.definition
  ...
  // Always add relations for nested filtering capability
  schema.definition
    ...
    .forEach((relation: any) => {
      ...
      let targetTableName = relation.targetTable?.name;
      if (!targetTableName && relation.targetTable?.id) {
        const targetTable = Object.values(schemas).find((s: any) => s.id === relation.targetTable.id);
        targetTableName = (targetTable as any)?.name;
      }
...
export function getFieldOptions(
  ...
  const field = schema.definition.find((f: any) => f.fieldType === 'column' && f.name === fieldName);
