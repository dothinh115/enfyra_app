...
const headerLayoutClasses = computed(() => {
  ...
  const justify = props.statsPosition === 'right' ? 'justify-between' : 'justify-start';
  const items = props.statsPosition === 'bottom' ? 'items-start' : 'items-center';
  const gap = props.statsPosition === 'bottom' ? 'gap-6' : '';
