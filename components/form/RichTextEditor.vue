...
<template>
  <div class="rich-text-editor">
    <!-- Loading State -->
    <div v-if="isLoading" class="flex items-center justify-center p-8 bg-gray-50 rounded-lg border-2 border-dashed border-gray-200">
      <div class="text-center">
        <div class="animate-spin rounded-full h-8 w-8 border-b-2 border-primary-600 mx-auto mb-3"></div>
        <p class="text-sm text-gray-600">Loading rich text editor...</p>
      </div>
    </div>
    ...
    <div v-else-if="isError" class="p-4 bg-red-50 border border-red-200 rounded-lg">
      <div class="flex">
        ...
        <div class="ml-3">
          <h3 class="text-sm font-medium text-red-800">Rich text editor failed to load</h3>
          <p class="text-sm text-red-700 mt-1">Please refresh the page to try again.</p>
        </div>
      </div>
    </div>
